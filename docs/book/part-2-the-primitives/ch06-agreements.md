...
When agreement terms change, you don't edit the agreement. You create a new version.
...
1. Agreement model with TWO explicit parties via GenericForeignKey
   - party_a and party_b (both can be any model)
   - party_a_id and party_b_id are CharField (not IntegerField) for UUID support

2. Terms must be stored as JSONField, not prose
   - Must support: plan_id, price_cents, currency, billing_cycle
   - Never store price as float

3. Version history via AgreementVersion model
   - Agreement FK, version number, terms snapshot, created_by, reason
   - Unique constraint on (agreement, version)
   - Never edit existing versions, only append new ones

4. Temporal validity
   - valid_from (required), valid_to (nullable means indefinite)
   - agreed_at (when decision was made), agreed_by (who authorized)
   - QuerySet methods: current(), as_of(timestamp), for_party(party)

5. Agreements are never deleted, only superseded or expired
...
Test your understanding by implementing a complete subscription flow:

```
Using the Agreement model from Exercise 2, implement:

1. create_subscription(customer, provider, plan_terms, started_by)
   - Creates Agreement and initial AgreementVersion
   - valid_from defaults to now, valid_to is None (indefinite)
   - Returns the Agreement

2. upgrade_subscription(agreement, new_plan_terms, changed_by, reason)
   - Creates new AgreementVersion with incremented version number
   - Does NOT modify the Agreement or previous versions
   - Returns the new AgreementVersion

3. cancel_subscription(agreement, cancelled_by, cancellation_date, reason)
   - Sets valid_to to cancellation_date
   - Creates new AgreementVersion recording the cancellation
   - Returns the updated Agreement

4. get_terms_as_of(agreement, timestamp)
   - Returns the terms that were effective at that timestamp
   - Returns None if agreement wasn't active at that time

Write tests covering:
- Creating a subscription
- Upgrading twice (version 1 → 2 → 3)
- Querying terms at various points in history
- Cancelling and confirming inactive status
- Idempotency (calling cancel twice should not create duplicate versions)
```

This exercise forces the AI to implement the full append-only versioning pattern correctly.
...
- Implement append-only versioning (AgreementVersion model)
...
### Must Not
- Never edit agreement terms (create new version instead)
- Never delete agreements (set valid_to instead)
...
- Agreements between same parties can overlap (use scope to distinguish)
```
...
1. **Models only one party** — The customer has a `subscription`, but who is the subscription with? The provider is implicit, breaking when you add resellers or multi-vendor scenarios.

2. **Uses mutable fields** — Plan changes update the subscription record. History is lost.

3. **Stores prices, not price references** — Terms include `"price": 49.99` instead of `"plan_id": "pro_monthly"`. When plans change, old records show stale data.

4. **Ignores temporal validity** — No valid_from/valid_to means you can't query historical state or handle future-dated agreements.

5. **Lacks version history** — The first time someone needs to prove what the terms were six months ago, the system fails.

6. **Uses FloatField for money in terms** — `"price": 49.99` stored as a float introduces rounding errors.

The fix is always explicit constraints. Tell the AI exactly what pattern to follow, and it will follow it consistently.
