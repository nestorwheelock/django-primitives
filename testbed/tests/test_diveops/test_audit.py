...
class TestBookingAudit:
    ...
    def test_book_excursion_emits_audit_event(
        self, dive_trip, diver_profile, audit_user, clear_audit_log
    ):
        """book_excursion() MUST emit BOOKING_CREATED audit event."""
    ...
    def test_book_excursion_audit_contains_required_metadata(
        self, dive_trip, diver_profile, audit_user, clear_audit_log
    ):
        """book_excursion() audit event MUST contain trip_id and diver_id in metadata."""
        from primitives_testbed.diveops.services import book_excursion
        ...
        assert "excursion_id" in metadata, "trip_id missing from audit metadata"
        assert "diver_id" in metadata, "diver_id missing from audit metadata"
        assert metadata["excursion_id"] == str(dive_trip.pk)
...
class TestCheckInAudit:
    ...
    def test_check_in_audit_contains_metadata(
        ...
        assert "excursion_id" in metadata
...
class TestTripStateAudit:
    ...
    def test_start_excursion_emits_audit_event(
        self, dive_trip, audit_user, clear_audit_log
    ):
        """start_excursion() MUST emit TRIP_STARTED audit event."""
        from primitives_testbed.diveops.services import start_excursion

        start_excursion(dive_trip, audit_user)
    ...
    def test_complete_excursion_emits_trip_completed_event(
        self, dive_trip, diver_profile, audit_user, clear_audit_log
    ):
        """complete_excursion() MUST emit TRIP_COMPLETED audit event."""
        ...
        complete_excursion(dive_trip, audit_user)
    ...
    def test_complete_excursion_emits_diver_completed_events(
        self, dive_trip, diver_profile, audit_user, clear_audit_log
    ):
        """complete_excursion() MUST emit DIVER_COMPLETED_TRIP for each roster entry."""
        from primitives_testbed.diveops.services import (
            book_excursion,
            check_in,
            complete_excursion,
...
class TestAuditSelectors:
    ...
    def test_diver_audit_feed_returns_diver_events(
        ...
        from primitives_testbed.diveops.services import add_certification, book_excursion

        # Create diver-related events
        add_certification(diver_profile, padi_open_water, audit_user)
        book_excursion(dive_trip, diver_profile, audit_user, skip_eligibility_check=True)
    ...
    def test_trip_audit_feed_returns_trip_events(
        ...
        from primitives_testbed.diveops.services import book_excursion, check_in, start_excursion
        ...
        assert len(feed) >= 3  # booking, check_in, start_excursion
    ...
    def test_trip_audit_feed_ordered_newest_first(
        ...
        from primitives_testbed.diveops.selectors import trip_audit_feed
        from primitives_testbed.diveops.services import book_excursion, start_excursion
